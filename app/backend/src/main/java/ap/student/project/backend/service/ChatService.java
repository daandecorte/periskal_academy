...
public class ChatService {
    ...
    public void addChatMember(ChatMemberDTO chatMemberDTO) {
        if(chatMemberDTO.chat_id()==0) {
            throw new MissingArgumentException("chat_id is missing");
        }
        if(chatMemberDTO.user_id()==0) {
            throw new MissingArgumentException("user_id is missing");
        }
        Chat chat = this.findById(chatMemberDTO.chat_id());
    ...
    public void addMessage(MessageDTO messageDTO) {
        ...
        Chat chat = this.findById(messageDTO.chat_id());
