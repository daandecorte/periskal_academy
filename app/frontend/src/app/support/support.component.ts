...
export class SupportComponent {
  ...
  async getUserId() {
    let userResponse = await fetch(
      `/api/users/periskal_id/${this.authService.currentUserValue?.ID}`
    );
    if (userResponse.ok) {
  ...
  async claimSupportTicket() {
    ...
    const checkChatMemberResponse = await fetch(
      `/api/chat/${this.currentChat.id}`
    );
    let chatMemberId = 0;
    ...
    if (chatMemberId == 0) {
      const memberResponse = await fetch(
        `/api/chat/${this.currentChat.id}/members`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
