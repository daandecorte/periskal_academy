...
export class TrainingOverviewComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (currentUserLocal) {
      let userResponse = await fetch(
        `/api/users/periskal_id/${currentUserLocal.ID}`
      );
  ...
  async getUserCertificate() {
    if (this.currentUser) {
      let userCertificateResponse = await fetch(
        `/api/user_certificates/training/${this.trainingId}/user/${this.currentUser.id}`
      );
  ...
  async getUserTraining() {
    if (this.currentUser) {
      let userTrainingResponse = await fetch(
        `/api/user_trainings/training/${this.trainingId}/user/${this.currentUser.id}`
      );
      ...
      if (userTrainingResponse.status == 404) {
        ...
        if (this.userTraining?.training_progress == null) {
          let trainingProgressPostResponse = await fetch(
          ...
          if (this.userTraining.training_progress.status != 'COMPLETED') {
            let trainingProgressPostResponse = await fetch(
              `/api/training_progress/${this.userTraining.training_progress.id}`,
              {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                  start_date_time:
                    this.userTraining.training_progress.start_date_time,
                  last_time_accessed: new Date().toISOString(),
                  status: 'IN_PROGRESS',
                  modules_completed:
                    this.userTraining.training_progress.modules_completed,
                }),
              }
            );
          } else {
  ...
  goToCertificate(): void {
    ...
    if (this.userTraining?.eligible_for_certificate == true) {
      ...
      if (this.training) {
        ...
        if (
          this.training.exam &&
          typeof this.training.exam === 'object' &&
          this.training.exam.id
        ) {
        ...
        // Check if exams is an array
          ...
          this.training.exams &&
          Array.isArray(this.training.exams) &&
          this.training.exams.length > 0
        ) {
