...
export class TrainingExamComponent implements OnInit, OnDestroy {
  ...
  updateLocalizedContent(): void {
    if (this.exam) {
      ...
      this.examDescription = this.getLocalizedContent(
        this.exam.descriptionLocalized
      );
    }
  }
  ...
  setCurrentQuestion(): void {
    ...
    if (this.currentQuestionIndex < this.questions.length) {
      ...
      if (this.shuffledQuestionsCache.has(this.currentQuestionIndex)) {
        this.currentQuestion = this.shuffledQuestionsCache.get(
        ...
        const shuffledQuestion = {
          ...baseQuestion,
          questionOptions: this.shuffleOptions([
            ...baseQuestion.questionOptions,
          ]),
        };
        this.shuffledQuestionsCache.set(
          this.currentQuestionIndex,
          shuffledQuestion
        );
      ...
      if (this.userAnswers.has(this.currentQuestion.id)) {
        this.selectedOptionId =
          this.userAnswers.get(this.currentQuestion.id) || null;
      } else {
  ...
  nextQuestion(): void {
    if (this.currentQuestionIndex < this.questions.length - 1) {
      ...
      // Update URL
      this.router.navigate(['/exams', this.examId, this.currentQuestionIndex], {
        replaceUrl: true,
      });
    } else {
  ...
  submitExam(): void {
    ...
    if (answers.length < this.totalQuestions) {
      console.warn(
        `Not all questions were answered. Expected: ${this.totalQuestions}, Got: ${answers.length}`
      );
    }
    ...
    this.examService.submitExam(examSubmission).subscribe({
      ...
      error: (error) => {
        ...
        this.examSubmissionError =
          'Exam submission failed, please check your internet connection.';
        this.isSubmitting = false;
      },
    });
  }
  ...
  private syncTimeWithBackend(): void {
    this.examService
      .getRemainingTime(this.examId, this.currentUserId)
      .subscribe({
